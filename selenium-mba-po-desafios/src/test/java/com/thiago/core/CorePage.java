...
public abstract class CorePage<T> {
	...
	public void arrastarElemento(WebElement elementoEmUso, WebElement elementoLivre ){
		Actions actions = new Actions(this.driver);
		Action dragAndDrop = null;
		dragAndDrop = actions.clickAndHold(elementoEmUso).moveToElement(elementoLivre).release().build();
		dragAndDrop.perform();
		try{
			Thread.sleep(1000);
		} catch(InterruptedException ie) {
			ie.printStackTrace();
			fail("Erro ao arrastar elemento pela tela. "+elementoEmUso);
			fail("Erro ao soltar elemento na tela. "+elementoLivre);
		}
	}
